...
# ==========================================
# 1. EoS LIBRARY GENERATOR
# ==========================================
def get_eos_library():
    """ Returns dictionary of Analytic Hadronic EoS models + Crust. """
    p = Symbol('p')
...
def solve_sequence(eos_input, is_quark):
    r_min = 1e-4
    # TOV Solver uses logspace for stability
    ...
    for pc in pressures:
        ...
        try:
            sol = solve_ivp(fun=lambda r, y: tov_rhs(r, y, eos_input, is_quark), 
                            t_span=(r_min, 30.0), y0=y0, events=surface_event, 
...
def worker_hadronic_gen(batch_size, baselines, seed_offset, batch_idx):
    ...
    valid_data = []
    attempts = 0
    # Data Gen: Try hard to fill batch
    while len(valid_data) < batch_size and attempts < batch_size * 500:
        attempts += 1
        nA, nB = np.random.choice(model_names, 2, replace=False)
        w = np.random.uniform(0.2, 0.8)
        base_max_m = w * baselines[nA] + (1-w) * baselines[nB]
        delta = np.random.uniform(-0.15, 0.05)
        ...
        try:
            ...
            f_R = interp1d(c_arr[:,0], c_arr[:,1])
            f_L = interp1d(c_arr[:,0], c_arr[:,2])
        ...
        for pt in curve:
            if pt[0] > 1.1 and pt[0] < max_m:
                valid_data.append([pt[0], pt[1], pt[2], 0, curve_id])
    return valid_data
    ...
    while len(valid_data) < batch_size and attempts < batch_size * 500:
        ...
        for pt in curve:
            if pt[0] > CONSTANTS['Q_M_MIN'] and pt[0] < max_m:
                valid_data.append([pt[0], pt[1], pt[2], 1, curve_id])
    return valid_data
...
# ==========================================
# 4. PLOTTING WORKER (Guaranteed Return)
# ==========================================
def worker_get_plot_curve(mode, baselines, seed):
    """
    Worker for plotting. 
    1. Guarantees return (while True).
    2. Returns Linear EoS grid (linspace) for Linear plotting.
    """
    np.random.seed(seed)
    ...
    if mode == 'hadronic':
        ...
        while True: # Keep trying until we find a valid curve
            nA, nB = np.random.choice(model_names, 2, replace=False)
            w = np.random.uniform(0.2, 0.8)
            base_m = w * baselines[nA] + (1-w) * baselines[nB]
            delta = np.random.uniform(-0.15, 0.05)
            alpha = ((base_m * (1.0 + delta)) / base_m)**2
            ...
            if max_m > CONSTANTS['H_M_MAX_LOWER'] and max_m < CONSTANTS['H_M_MAX_UPPER']:
                ...
                try:
                    c = c[c[:,0].argsort()]
                    f_R = interp1d(c[:,0], c[:,1])
                    f_L = interp1d(c[:,0], c[:,2])
                    
                    if f_R(1.4) >= CONSTANTS['H_R14_MIN'] and f_L(1.4) <= CONSTANTS['H_L14_MAX']:
                        # --- LINEAR SAMPLING FOR LINEAR PLOT ---
                        p_max = np.max(c[:,3])
                        # LINSPACE is critical for linear plot quality
                        p_grid = np.linspace(1.0, p_max, 300) 
                        
                        eps_grid = []
                        for p_val in p_grid:
                            # Evaluate Core physics directly (skipping crust conditional)
                            p_base = p_val / alpha
                            vA, vB = fA_e(p_base), fB_e(p_base)
                            if vA > 0 and vB > 0:
                                e_base = (vA**w) * (vB**(1.0-w))
                                eps_grid.append(e_base * alpha)
                            else:
                                eps_grid.append(np.nan)
                        
                        eps_grid = np.array(eps_grid)
                        valid_idx = ~np.isnan(eps_grid)
                        eos_arr = np.column_stack((eps_grid[valid_idx], p_grid[valid_idx]))
                        return (c, eos_arr)
                except: continue
    else:
        # Quark
        while True:
            ...
            if max_m > 1.4 and max_m < CONSTANTS['H_M_MAX_UPPER']:
                ...
                if np.max(c[:,1]) <= CONSTANTS['Q_R_MAX']:
                    ...
                    p_max = np.max(c[:,3])
                    p_grid = np.linspace(1.0, p_max, 300)
                    eps_grid = (p_grid / cs2) + (4.0 * B)
                    eos_arr = np.column_stack((eps_grid, p_grid))
                    return (c, eos_arr)